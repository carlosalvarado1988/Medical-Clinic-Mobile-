    ...
    <body>
          ...
          <div data-role="content">
                    ...
                    <br/></p><p><input type='submit' value='Enviar'/></p>";
                        ...
                        $link = conectar();
                        $exp = $_GET['exp'];
                        $queryx = "SELECT * FROM controlmedicamento AS c INNER JOIN historialclinico AS h ON c.NumHistorialClinico=h.NumHistorialClinico AND h.NumExpediente = $exp ORDER BY h.FechaConsulta DESC";
                        ...
                        if ($row = mysql_fetch_array($resultx)) {
                            ...
                            echo "<p><b>Medicamento recetado a " . $nombre . " " . $apellido . "</b></p><p><a href='consulMedicamento.php'>Ver otro expediente</a></p>";
                            echo "<table class='zebra'><thead><tr><th nowrap='nowrap'>Historial Clinico</th><th nowrap='nowrap'>N° de Medicamento</th><th nowrap='nowrap'>Dosis de Medicamento</th><th nowrap='nowrap'>Cantidad Recetada</th></tr></thead><tbody>";
                            do {
                                echo "<tr><td>" . $row["NumHistorialClinico"] . "</td><td>" . $row["NumMedicamento"] . "</td><td>" . $row["DosisMedicamento"] . "</td><td>" . $row["CantMedicamento"] . "</td></tr>";
                            } while ($row = mysql_fetch_array($resultx));
                            echo '</tbody></table>';
                        } else {
                            echo "<p><b>No se ha encontrado ningun registro</b><p><p><a href='consulMedicamento.php'>Reintentar</a></p>";
                        }
                    }
                    ?>
              
			  <a href="RegMedicamento.php"><button type="button" data-inline="true">Registrar Receta</button></a>
            </div>
        </div>    
